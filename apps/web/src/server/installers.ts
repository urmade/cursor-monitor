...
function posixInstaller(platform: 'linux' | 'macos'): InstallerArtifact {
  ...
  const startScript = [
    ...
    'date -u +%Y-%m-%dT%H:%M:%SZ >"$STATE_DIR/started-at" 2>/dev/null',
    "printf '%s\\n' '{\"continue\":true}'",
    'exit 0',
  ].join('\n');
  const stopScript = [
    ...
    'fi',
    'STARTED_AT=$(tr -d "\\r\\n" <"$STATE_DIR/started-at" 2>/dev/null)',
    'rm -f "$STATE_DIR/started-at" 2>/dev/null',
  ...
  const content = [
    ...
    '# Uses only the OS shell, curl, and optional git; no package installation.',
    'set -eu',
    'ROOT="${1:-$PWD}"',
    'HOOK_DIR="$ROOT/.cursor/hooks"',
    'mkdir -p "$HOOK_DIR"',
    `cat >"$ROOT/.cursor/hooks.json" <<'CURSOR_MONITOR_HOOKS'`,
    hooksJson,
  ...
  return {
    ...
    requirements:
      platform === 'macos'
        ? 'macOS built-in /bin/sh and /usr/bin/curl; git is optional.'
        : 'A standard Linux /bin/sh and curl; git is optional.',
  };
}
...
function windowsInstaller(): InstallerArtifact {
  ...
  const startScript = [
    ...
    "(Get-Date).ToUniversalTime().ToString('o') | Set-Content -Encoding UTF8 (Join-Path $stateDir 'started-at')",
    "Write-Output '{\"continue\":true}'",
    'exit 0',
  ].join('\r\n');
  const stopScript = [
    "$ErrorActionPreference = 'SilentlyContinue'",
    `$endpoint = ${powershellQuote(endpoint)}`,
    `$token = ${powershellQuote(token)}`,
    `$bypass = ${powershellQuote(bypass)}`,
    "$stateDir = Join-Path $HOME '.cursor\\cursor-monitor'",
    'New-Item -ItemType Directory -Force -Path $stateDir | Out-Null',
    ...
    '}',
    "$startedFile = Join-Path $stateDir 'started-at'",
  ...
  const content = [
    ...
    `$startScript = @'\r\n${startScript}\r\n'@`,
    `$stopScript = @'\r\n${stopScript}\r\n'@`,
    "$hooksJson | Set-Content -Encoding UTF8 (Join-Path $root '.cursor\\hooks.json')",
    "$startScript | Set-Content -Encoding UTF8 (Join-Path $hookDir 'cursor-monitor-start.ps1')",
    "$stopScript | Set-Content -Encoding UTF8 (Join-Path $hookDir 'cursor-monitor-stop.ps1')",
